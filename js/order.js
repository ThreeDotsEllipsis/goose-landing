...
function createImages(type) {
    ...
    for (i = 0; i < len; i++) {
        ...
        radio.id = i;
        trigger.appendChild(radio);

        trigger.addEventListener("click", () => {
            let shiftAmount = document.querySelector(".image-view").offsetWidth;
            imageLine.style.left = `-${shiftAmount * radio.id}px`;
            radio.checked = true;
        });

        imageIndex.appendChild(trigger);
...
window.addEventListener("resize", () => {
    animateUnderline(
        document.querySelector(".size input:checked"),
        sizeUnderline
    );
    animateUnderline(
        document.querySelector(".color input:checked"),
        colorUnderline
    );
